...
The shell theme has to change the accent colors of the control popout (upper right corner) too. You can take screenshots of the buttons when it's activated, hovered (when activated) and pressed down (when activated) with the theme turned off and use [Eyedropper](https://apps.gnome.org/Eyedropper/) (or any alternative) on the screenshots to get the necessary colors.
